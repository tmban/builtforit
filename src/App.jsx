...
export default function App() {
  ...
  const questions = selectedPath ? QUESTIONS[selectedPath] : [];
  const totalQ = questions.length;
  const pathObj = selectedPath ? ALL_PATHS.find(p => p.id === selectedPath) : null;
  const isCareer = selectedPath ? CAREER_PATHS.some(p => p.id === selectedPath) : false;

  const d = darkMode;
  const c = {
    bg: d ? "#0a0a0a" : "#f7f5f2",
    surface: d ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)",
    surfaceHover: d ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.07)",
    border: d ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.12)",
    text: d ? "#f5f2ee" : "#111111",
    textMuted: d ? "rgba(255,255,255,0.65)" : "rgba(0,0,0,0.65)",
    textFaint: d ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.38)",
    accent: "#ff6b35",
    accentBg: d ? "rgba(255,107,53,0.10)" : "rgba(255,107,53,0.08)",
    modalBg: d ? "#141414" : "#ffffff",
  };
  ...
  function switchMode(m) { setMode(m); setSelectedPath(null); }
  ...
  function startQuestions() { transition(() => { setCurrentQ(0); setAnswers({}); setSelectedOpt(null); setScreen("questions"); }); }
  function goBack() { transition(() => { setScreen("home"); setSelectedOpt(null); }); }

  function nextQuestion() {
    const q = questions[currentQ];
    const na = selectedOpt ? { ...answers, [q.tag]: selectedOpt } : answers;
    setAnswers(na);
    setSelectedOpt(null);
...
Only include advice for tags where the score is below 60. Max 4 advice items. If all scores are high, return an empty array.`;
  ...
  function reset() {
    transition(() => { setScreen("home"); setSelectedPath(null); setCurrentQ(0); setAnswers({}); setSelectedOpt(null); setVerdict(null); setShowAnswers(false); });
  }
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: c.bg, minHeight: "100vh", color: c.text, transition: "background 0.3s, color 0.3s" }}>
      <style>{`
        ...
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, backdropFilter: "blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} className="pop" style={{ background: c.modalBg, borderRadius: 20, border: `1px solid ${c.border}`, padding: "26px 26px 22px", maxWidth: 400, width: "100%", maxHeight: "88vh", overflowY: "auto" }}>
            ...
            {modal === "howtoplay" && (<>
              ...
              {[["Pick your path", "Scroll through the scenarios and career paths. Pick the one you'd most want to make it in — or the one you're most curious about."],
            ...
            {modal === "stats" && (<>
              ...
              {sectionLabel("Verdict breakdown")}
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
                {[["PASS", stats.passes, "#4ade80"], ["MAYBE", stats.maybes, "#fbbf24"], ["STRUGGLE", stats.fails, "#f87171"]].map(([label, count, col]) => {
                  const pct = stats.played > 0 ? Math.round((count / stats.played) * 100) : 0;
                  return (
                    <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 40, fontSize: "0.65rem", fontWeight: 700, color: c.textFaint, textAlign: "right", letterSpacing: "0.04em" }}>{label}</div>
                      <div style={{ flex: 1, height: 26, background: c.surface, borderRadius: 5, overflow: "hidden", position: "relative" }}>
                        <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: (pct || 0) + "%", background: col, borderRadius: 5, minWidth: count > 0 ? 26 : 0, transition: "width 0.9s ease", display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: 7 }}>
                          {count > 0 && <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "#000" }}>{count}</span>}
                        </div>
                        {count === 0 && <div style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", fontSize: "0.75rem", color: c.textFaint }}>0</div>}
                      </div>
                    </div>
                  );
                })}
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "0 22px" }}>

        {/* HEADER */}
        <header style={{ padding: "22px 0 18px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${c.border}`, marginBottom: 30 }}>
        ...
        {screen === "home" && (
          <div className={animIn ? "screen" : "screen-out"}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 22 }}>
              {ALL_PATHS.map(p => (
                <button key={p.id} onClick={() => setSelectedPath(p.id)} style={{ padding: 16, borderRadius: 14, border: `1px solid ${selectedPath === p.id ? c.accent : c.border}`, background: selectedPath === p.id ? c.accentBg : c.surface, cursor: "pointer", textAlign: "left", display: "flex", flexDirection: "column", gap: 8, transition: "all 0.18s", transform: selectedPath === p.id ? "translateY(-1px)" : "none" }}>
                  <span style={{ fontSize: "1.3rem", lineHeight: 1 }}>{p.icon}</span>
                  <span style={{ fontFamily: "'Syne', sans-serif", fontSize: "0.78rem", fontWeight: 700, color: c.text, lineHeight: 1.3 }}>{p.label}</span>
                  <span style={{ fontSize: "0.66rem", color: selectedPath === p.id ? c.accent : c.textFaint }}>{p.sub}</span>
                </button>
              ))}
            </div>
            <button onClick={startQuestions} disabled={!selectedPath} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: selectedPath ? c.accent : (d ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"), color: selectedPath ? "#fff" : c.textFaint, fontFamily: "'Syne', sans-serif", fontSize: "0.88rem", fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", transition: "all 0.2s", cursor: selectedPath ? "pointer" : "not-allowed" }}>
              Let's go →
            </button>
          </div>
        )}
        ...
        {screen === "questions" && (
          <div className={animIn ? "screen" : "screen-out"} key={currentQ}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={nextQuestion} disabled={!selectedOpt} style={{ padding: "12px 28px", borderRadius: 10, border: "none", background: selectedOpt ? c.accent : (d ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"), color: selectedOpt ? "#fff" : c.textFaint, fontFamily: "'Syne', sans-serif", fontSize: "0.82rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", transition: "all 0.2s", cursor: selectedOpt ? "pointer" : "not-allowed" }}>
                        ...
                        {Object.entries(answers).map(([tag, val], i, arr) => {
                          ...
                          return (
                            <div key={tag} style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 16px", background: i % 2 === 0 ? c.surface : "transparent", borderBottom: i < arr.length - 1 ? `1px solid ${c.border}` : "none" }}>
                              ...
                              <span style={{ fontSize: "0.82rem", color: c.textMuted, flex: 1 }}>{val}</span>
                              {s !== undefined && (
