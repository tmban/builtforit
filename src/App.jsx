...
The user wants to be assessed on: "${customPath}"

Generate a research-backed set of questions for this path. Use your knowledge to assess whether someone is genuinely built for this.

Return ONLY raw JSON, no markdown:
{
  "label": "<clean path name>",
  "icon": "<single relevant emoji>",
  "sub": "<5-7 word description of what this path involves>",
  "questions": {
    "multiple_choice": [
...
Mix questions across: current ability, daily routine/habits, and scenario responses. Be specific to this exact path.`;
    ...
    try {
      ...
      // Interleave: insert open-ended at positions 5 and 11
      const combined = [...mcQs];
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: c.bg, minHeight: "100vh", color: c.text, transition: "background 0.3s, color 0.3s" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 28px" }}>
        ...
        {/* HEADER */}
        <header style={{ padding: "40px 0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${c.border}`, marginBottom: 36 }}>
          ...
          <div className={animIn ? "screen" : "screen-out"}>
            ...
            {!nameSubmitted ? (
              <div style={{ marginBottom: 36 }}>
                ...
                <div style={{ fontSize: "0.85rem", color: c.textMuted, marginBottom: 18 }}>First name or nickname — we'll use it in your verdict.</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    value={nameInput}
                    onChange={e => setNameInput(e.target.value)}
                    onKeyDown={e => { if (e.key === "Enter" && nameInput.trim()) { setUserName(nameInput.trim()); setNameSubmitted(true); } }}
                    placeholder="e.g. Thomas, T, Tommo..."
                    style={{ flex: 1, padding: "13px 16px", borderRadius: 11, border: `1.5px solid ${c.border}`, background: c.surface, color: c.text, fontSize: "0.92rem", outline: "none", fontFamily: "'DM Sans', sans-serif" }}
                    autoFocus
                  />
                  <button
                ...
                <div style={{ marginTop: 10 }}>
                  <button onClick={() => { setUserName(""); setNameSubmitted(true); }} style={{ background: "none", border: "none", color: c.textFaint, fontSize: "0.75rem", textDecoration: "underline", textUnderlineOffset: 3, cursor: "pointer", padding: 0 }}>
                    Skip — just take me to the paths
                  </button>
                </div>
              </div>
            ) : (
              <>
                ...
                {userName && (
                  <div style={{ marginBottom: 32 }}>
                    ...
                    <div style={{ fontSize: "0.88rem", color: c.textMuted, marginTop: 8 }}>Pick a path or type your own. No sugarcoating.</div>
                  </div>
                )}

                {/* Custom path — prominent, at top */}
                <div style={{ marginBottom: 28 }}>
                  <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.95rem", color: c.text, marginBottom: 6 }}>Type your own path</div>
                  <div style={{ fontSize: "0.78rem", color: c.textMuted, marginBottom: 12 }}>Any career, lifestyle, or challenge — we'll build the questions around it.</div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <input
                      value={customPath}
                      onChange={e => { setCustomPath(e.target.value); if (selectedPath !== "__custom__") setSelectedPath(null); }}
                      onFocus={() => setSelectedPath(null)}
                      onKeyDown={e => { if (e.key === "Enter" && customPath.trim() && !generatingQuestions) { setSelectedPath("__custom__"); generateCustomQuestions().then(() => startQuestions()); } }}
                    ...
                    />
                    <button
                      onClick={async () => { setSelectedPath("__custom__"); await generateCustomQuestions(); startQuestions(); }}
                  ...
                  {generatingQuestions && (
                    <div style={{ fontSize: "0.75rem", color: c.textMuted, marginTop: 8 }}>
                      Building questions for {customPath}...
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                  ...
                  <div style={{ fontSize: "0.7rem", color: c.textFaint, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>or choose a path</div>
                  <div style={{ flex: 1, height: 1, background: c.border }} />
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {[
                    ...
                    >
                      ...
                      <div style={{ position: "absolute", bottom: 14, right: 16, fontSize: "0.7rem", color: c.textFaint, fontWeight: 600, letterSpacing: "0.06em" }}>TAP →</div>
                    </button>
                  ))}
                </div>
              </>
