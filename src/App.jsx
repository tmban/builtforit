    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: c.bg, minHeight: "100vh", color: c.text, transition: "background 0.3s, color 0.3s", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 600px) {
          ...
          .header-icons-mobile { display: flex !important; gap: 12px !important; }
          ...
          /* Answer row: pin tag left, score right */
        ...
        {screen === "home" && (
          <div className={animIn ? "screen" : "screen-out"}>
            ...
            {!nameSubmitted ? (
              ...
              <>
                {/* Greeting */}
                {/* Greeting — only shown when name was provided */}
                {userName && (
                ...
                <div style={{ marginBottom: 40 }}>
                  {!userName && (
                    <>
                      <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "1.8rem", color: c.text, lineHeight: 1.15, marginBottom: 8 }}>
                        <span style={{ color: c.accent }}>Are you actually built for it?</span>
                      </div>
                      <div style={{ fontSize: "0.88rem", color: c.textMuted, marginBottom: 16 }}>Pick a dream or type your own. Any career, lifestyle, or challenge. No sugarcoating.</div>
                    </>
                  )}
                  <div className="custom-input-row" style={{ display: "flex", gap: 8 }}>
                    <input
            ...
            {loading ? (
              ...
              return (
                <>
                    ...
                    <>
                      ...
                      <div style={{ borderRadius: 12, border: `1px solid ${c.border}`, overflow: "hidden", marginBottom: 20 }}>
                        {Object.entries(answers).map(([tag, val], i, arr) => {
                          ...
                          return (
                            <div key={tag} className="answer-row" style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 16px", background: i % 2 === 0 ? c.surface : "transparent", borderBottom: i < arr.length - 1 ? `1px solid ${c.border}` : "none" }}>
                  ...
                  <button onClick={reset} style={{ width: "100%", padding: 14, borderRadius: 12, border: "none", background: c.accent, color: "#fff", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "0.88rem", letterSpacing: "0.04em", textTransform: "uppercase", transition: "all 0.2s", cursor: "pointer" }}>
      ...
      <footer style={{ borderTop: `1px solid ${c.border}`, padding: "24px 28px", marginTop: "auto" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: "0.78rem", color: c.textFaint }}>
            Built by{" "}
            <a href="https://thomasmbanefo.com" target="_blank" rel="noopener noreferrer"
              style={{ color: c.textFaint, fontWeight: 600, textDecoration: "none" }}>
              Vivid Vault Studios
            </a>
            {" "}· © 2025
          </div>
          <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
            <button onClick={() => setModal("privacy")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.75rem", color: c.textFaint, fontWeight: 500, padding: 0 }}>Privacy policy</button>
            <button onClick={() => setModal("terms")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.75rem", color: c.textFaint, fontWeight: 500, padding: 0 }}>Terms of service</button>
          </div>
        </div>
      </footer>
    </div>
