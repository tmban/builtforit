...
const ALLOWED_ORIGINS = [
  "https://builtforit.vercel.app",
  "https://www.builtforit.vercel.app",
];
...
export default async function handler(req, res) {
  ...
  if (isRateLimited(ip)) {
    return res.status(429).json({ error: "Too many requests. Take a breather and try again in an hour." });
  }
  ...
  try {
    const { messages, max_tokens = 800, tools } = req.body;
    ...
    const safeTokens = Math.min(Number(max_tokens) || 800, 2500);
